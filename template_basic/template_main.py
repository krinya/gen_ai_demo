...
# Local imports
from prompts.router_prompts import ROUTER_PROMPT, ANSWER_GRADING_PROMPT, QUESTION_REPHRASE_PROMPT
...
class ChatbotState(TypedDict):
    ...
    route_decision: str # Decision made by the router (faq, rag, llm)
    current_answer: str # The answer generated by the current source
    answer_quality: str # Quality of the answer (good, bad)
    ...
    current_source: str  # Track current source (faq, rag, llm)
    failed_sources: List[str]  # Track which sources have failed
...
class TemplateChatbot:
    ...
    def _load_environment(self):
        ...
        if langsmith_api_key:
            ...
            os.environ["LANGCHAIN_PROJECT"] = "template-chatbot"
    ...
    async def _route_question(self, state: ChatbotState) -> ChatbotState:
        """Route the question to appropriate knowledge source"""
        ...
        # Clean the decision and validate
        route_decision = route_decision.strip().upper()
        if route_decision not in ["FAQ", "RAG", "LLM"]:
            route_decision = "LLM"  # Default fallback
        
        # If this source already failed 3 times, try alternative
        current_source_lower = route_decision.lower()
        if failed_sources.count(current_source_lower) >= 3:
            alternative_sources = [s for s in ["faq", "rag", "llm"] if s != current_source_lower]
            if alternative_sources:
                # Try the first alternative that hasn't failed as much
                for alt in alternative_sources:
                    if failed_sources.count(alt) < 3:
                        route_decision = alt.upper()
                        logger.info(f"Switching to alternative source: {route_decision} (original failed {failed_sources.count(current_source_lower)} times)")
                        break
        ...
        return {
            **state, 
            "route_decision": route_decision.lower(),
            "current_source": route_decision.lower(),
            "context": context
        }
    ...
    async def _grade_answer(self, state: ChatbotState) -> ChatbotState:
        ...
        if "GOOD" in grade_response.upper():
            quality = "good"
        else:
            quality = "bad"
        
        logger.info(f"Answer graded as: {quality}")
    ...
    async def _rephrase_question(self, state: ChatbotState) -> ChatbotState:
        ...
        if attempts > self.max_rephrase_attempts:
    ...
    async def _final_response(self, state: ChatbotState) -> ChatbotState:
        ...
        if state.get("rephrase_attempts", 0) >= self.max_rephrase_attempts and state["answer_quality"] == "bad":
            final_answer = (
    ...
    def _grade_decision(self, state: ChatbotState) -> str:
        ...
        if rephrase_attempts >= self.max_rephrase_attempts:
...
async def create_chatbot(session_id: Optional[str] = None) -> TemplateChatbot:
    ...
    return TemplateChatbot(session_id=session_id)
